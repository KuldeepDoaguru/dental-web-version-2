...
const ViewPrescription = () => {
  ...
  const getTreatDetails = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-superadmin.vimubds5.a2hosted.com/api/v1/super-admin/getTreatmentDataList/${tpid}/${branch.name}`,
        {
          headers: {
  ...
  const getTreatPrescriptionByAppointId = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-superadmin.vimubds5.a2hosted.com/api/v1/super-admin/getTreatPrescriptionByAppointIdList/${tpid}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${user.token}`,
  ...
  const getEmployeeData = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://dentalguru-superadmin.vimubds5.a2hosted.com/api/v1/super-admin/getEmployeeDetails/${branch.name}/${getPatientData[0]?.doctor_id}`,
        {
          headers: {
            "Content-Type": "multipart/form-data",
            Authorization: `Bearer ${user.token}`,

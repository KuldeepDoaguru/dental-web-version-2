...
const ClinicActivity = () => {
  ...
  const getPatdetailsByBranch = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-admin.vimubds5.a2hosted.com/api/v1/admin/getPatientDetailsByBranch/${branch}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${user.token}`,
  ...
  const date = String(tdate.getDate()).padStart(2, "0");

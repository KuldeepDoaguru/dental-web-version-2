...
const EmployeeProfile = () => {
  ...
  return (
    ...
    {["receptionist", "accountant", "lab attendant", "doctor"]
      .filter((role) => inEmpData.empDesignation === "doctor" || role !== "doctor")
      .map((role) => (
        <div className="form-check" key={role}>
          <input
            className="form-check-input"
            ...
            onChange={handleCheckChange}
          />
          <label className="form-check-label" htmlFor={`flexCheck${role}`}>
            {role.charAt(0).toUpperCase() + role.slice(1)}
          </label>
        </div>
      ))}

...
const getChiefComplain = (req, res) => {
  try {
    const selectQuery = "SELECT * FROM chief_complain_list";
    db.query(selectQuery, (err, result) => {
      if (err) {
        res.status(400).json({ success: false, message: err.message });
      }
...
const addChiefComplainTOList = (req, res) => {
  const dateTime = moment().tz("Asia/Kolkata").format("DD-MM-YYYY HH:mm:ss");
  try {
    const { complain } = req.body;
    const selectQuery =
      "SELECT * FROM chief_complain_list WHERE chief_complain = ?";
    db.query(selectQuery, complain, (err, result) => {
      if (err) {
        res.status(400).json({ success: false, message: err.message });
      }
      if (result.length > 0) {
        res
          .status(400)
          .json({ success: false, message: "chief complain already exists" });
      } else {
        const insertQuery =
          "INSERT INTO chief_complain_list (chief_complain, created_time) VALUES (?, ?)";
        db.query(insertQuery, [complain, dateTime], (err, result) => {
          if (err) {
            res.status(400).json({ success: false, message: err.message });
          }
          res.status(200).json({
            success: true,
            message: "chief complain added successfully",
          });
        });
      }
    });

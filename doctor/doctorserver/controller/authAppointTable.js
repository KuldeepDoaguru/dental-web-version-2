...
const getPatientByAppID = (req, res) => {
  const appoint_id = req.params.appoint_id;

  const sql = `SELECT * FROM appointments JOIN patient_details ON patient_details.uhid = appointments.patient_uhid WHERE appointments.appoint_id = ?`;
  ...
  db.query(sql, appoint_id, (err, result) => {
    if (err) {
      ...
      return res.status(200).json({ message: "Successfully Get Data", result });
    }
  });
};

const insertPatientPrescription = (req, res) => {
  const {
    name,
    uhid,
    appoint_id,
  ...
  const sql = `
    INSERT INTO quick_prescription (
      name, uhid, appoint_id, age, gender, date, chief_complain, 
      medical_histry, dental_histry, onexam, advice, medicines
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        res.status(200).send("Prescription inserted successfully");
      }
    }
  );
};

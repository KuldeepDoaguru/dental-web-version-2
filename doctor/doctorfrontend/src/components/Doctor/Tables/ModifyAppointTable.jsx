...
const ModifyAppointTable = () => {
    ...
    const fetchAppointments = async () => {
      try {
        const res = await axios.get(
          `http://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getAppointmentsWithPatientDetails?date=${selectedDate}`
        );
  ...
  const handleAction = async (action, appointId) => {
    try {
      ...
      await axios.put(
        `http://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/upDateAppointmentStatus`,
        requestBody
      );
                        ...
                        <ul className="dropdown-menu">
                          <li>
                            <button
                              ...
                              onClick={() =>
                                handleAction("In Treatment", item.appoint_id)
                              }
                            >
                              Start Treatment
                            </button>
                          ...
                          <li>
                            <button
                              className="dropdown-item mx-0"
                              onClick={() =>
                                handleAction("Cancel", item.appoint_id)
                              }
                            >
                              Cancel Treatment
                            </button>
                          ...
                          <li>
                            <button
                              className="dropdown-item mx-0"
                              onClick={() =>
                                handleAction("On Hold", item.appoint_id)
                              }
                            >
                              Hold
                            </button>

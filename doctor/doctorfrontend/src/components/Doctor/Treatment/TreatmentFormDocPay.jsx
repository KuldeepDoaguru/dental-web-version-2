...
const TreatmentFormDocPay = () => {
  ...
  const getDentalTreatData = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getOnlyExaminv/${tp_id}/${tsid}`,
        {
          headers: {
  ...
  const getTreatmentList = async () => {
    try {
      ...
      if (sitCheck.length > 0) {
        endpoint = `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getExaminedataByIdandexamineAfterSitOne/${tsid}/${tp_id}`;
      } else {
        endpoint = `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getExaminedataByIdandexamine/${tsid}/${tp_id}`;
      }
  ...
  const treatmentStatsUpdate = async () => {
    try {
      const response = await axios.put(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/updateTreatSittingStatus/${branch}/${tsid}`,
  ...
  const generateBillSitting = async () => {
    try {
      const res = await axios.post(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/generateSittingBill/${tp_id}/${branch}`,
        sittingForm
      );
  ...
  const updateAppointStatus = async () => {
    try {
      const res = await axios.put(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/updateAppointmentStatusAfterTreat/${appoint_id}`,
        {
          status: "Complete",
        },
        {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/insertTreatmentData/${tsid}/${appoint_id}/${tp_id}`,
        formDetails,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const getPatientDetail = async () => {
    try {
      const res = await axios.get(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getAppointmentsWithPatientDetailsById/${tp_id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const getSecurityAmt = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/getSecurityAmountByAppointmentId/${tp_id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const updateAmountAfterPayViaSecAmount = async () => {
    try {
      const res = await axios.put(
        `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/updateRecSecAmountAfterPayment/${tp_id}`,
        {
          sec_rec_amt:
  ...
  const MakePaymentViaSecurityAmount = async () => {
    if (formData.paid_amount === "") {
      ...
      try {
        const res = await axios.put(
          `https://dentalguru-doctor.vimubds5.a2hosted.com/api/doctor/updateSecurityAmountAfterPayment/${tp_id}`,
...
export default TreatmentFormDocPay;
